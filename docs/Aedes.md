...
- options `<object>`
  ...
  - `topicAliasMaximum` `<number>` MQTT 5.0 only. Maximum __inbound__ Topic Alias value the broker accepts from a client, advertised in the CONNACK. `0` disables inbound topic aliases. __Default__: `0`. _(The reverse direction is automatic and needs no option: when a client advertises its own Topic Alias Maximum in CONNECT, the broker assigns aliases on __outbound__ PUBLISH to that client — full topic + alias on first use, empty topic + alias thereafter, per connection.)_
