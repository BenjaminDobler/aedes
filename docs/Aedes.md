...
- options `<object>`
  ...
  - `topicAliasMaximum` `<number>` MQTT 5.0 only. Maximum __inbound__ Topic Alias value the broker accepts from a client, advertised in the CONNACK. `0` disables inbound topic aliases. __Default__: `0`. (Outbound aliasing — the broker aliasing topics on PUBLISHes it sends — is automatic and needs no option; see the callout under [`Event: connackSent`](#event-connacksent).)
