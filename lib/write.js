...
function withOutboundTopicAlias (client, packet, version) {
  ...
  // own aliasing on top. Only for a non-empty string topic: an empty-topic packet
  // carrying a caller alias is the embedder's own reuse form (via client.publish()) —
  // stripping there would emit a zero-length topic with no alias, a malformed frame
  // [MQTT-3.3.2-1], so leave that packet untouched.
  const callerAlias = version === 5 && packet.cmd === 'publish' &&
    typeof packet.topic === 'string' && packet.topic !== '' &&
  ...
  if (aliases.size >= max) {
    // Alias table full — send the full topic name, no alias (never evict). Scrub a
    // stray caller alias.
    return callerAlias ? stripTopicAlias(packet) : packet
  }
