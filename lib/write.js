  ...
  if (client.connecting || client.connected) {
    try {
      ...
      if (!result && !client.errored) {
        // mqtt-packet's auth() writer returns false ONLY when the packet cannot
        // fit the client's Maximum Packet Size (it writes nothing) — never on
        // backpressure (it returns true after writing). Treat that as a send
        // failure [MQTT-3.1.2-25], not a drain wait, which would hang the exchange
        // until connectTimeout. Every other packet's false is genuine backpressure.
        if (packet.cmd === 'auth') {
          error = new Error('AUTH exceeds the client Maximum Packet Size')
        } else {
