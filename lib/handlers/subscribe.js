...
import { through, validateTopic, $SYS_PREFIX, runFall, runParallel, once, ackProperties } from '../utils.js'
import { ReasonCodes, SUBSCRIPTION_IDENTIFIER_MAX, REASON_CODE_ERROR_THRESHOLD } from '../constants.js'
import write from '../write.js'

// [MQTT-4.8.2] Shared Subscription topic-filter prefix.
...
// Derive a single packet-level Reason String from the per-topic granted codes
// (v5 SUBACK carries one reasonString for the whole packet). The per-topic codes
// still carry the machine-readable detail; this is human-readable diagnostics.
function subackReasonString (granted) {
  if (granted.some(code => code === ReasonCodes.SHARED_SUBSCRIPTIONS_NOT_SUPPORTED)) {
    return 'shared subscriptions are not supported'
  }
  if (granted.some(code => code === ReasonCodes.NOT_AUTHORIZED)) {
    return 'not authorized to subscribe'
  }
  if (granted.some(code => code >= REASON_CODE_ERROR_THRESHOLD)) {
...
class SubAck {
  constructor (packet, granted, client) {
    this.cmd = 'suback'
    this.messageId = packet.messageId
    // the qos granted
    this.granted = granted
    // [#823] Reason String on a failed subscription, gated by Request Problem
    // Information.
    const properties = ackProperties(client, subackReasonString(granted))
...
class SubState {
  constructor (client, packet, granted, rh, rap, nl) {
    ...
    this.rap = rap
    this.nl = nl
    // Retain Handling 1 sends retained messages only for a genuinely new
    // subscription. Default false so the success path can only opt *in*: if
    // addSubs short-circuits (e.g. client closing) before computing this, a
    // closing-client rh:1 resubscribe can't trigger stray retained delivery.
...
function doSubscribe (sub, done) {
  ...
  if (this.client.version === 5 && sub.topic.startsWith(SHARE_PREFIX)) {
...
function addSubs (sub, done) {
  ...
  const existing = client.subscriptions[topic]
  this.isNewSubscription = !existing
  if (!existing) {
    client.subscriptions[topic] = new Subscription(qos, func, rh, rap, nl, si)
    broker.subscribe(topic, func, done)
  } else if (existing.qos !== qos || existing.rh !== rh || existing.rap !== rap || existing.nl !== nl || existing.si !== si) {
    const oldFunc = existing.func
    client.subscriptions[topic] = new Subscription(qos, func, rh, rap, nl, si)
...
function completeSubscribe () {
  ...
  if (packet.messageId !== undefined) {
    // [MQTT-3.9.3-1]
    write(client,
      new SubAck(packet, this.subState.map(obj => obj.granted), client),
      done)
  } else {
    done()
  }
  ...
  for (let i = 0; i < subs.length; i++) {
    ...
    if (sub.granted < REASON_CODE_ERROR_THRESHOLD && sub.rh !== 2 && (sub.rh !== 1 || sub.isNewSubscription)) {
