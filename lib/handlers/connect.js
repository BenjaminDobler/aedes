...
import { SESSION_NEVER_EXPIRES, RECEIVE_MAXIMUM_DEFAULT, OUTBOUND_TOPIC_ALIAS_LIMIT, ReasonCodes } from '../constants.js'
...
function init (client, packet, done) {
  ...
  // Only a positive number is honored. mqtt-packet decodes a duplicated property
  // into an array (a Protocol Error per §3.1.2.11.5); a preConnect hook could also
  // set a negative value on the mutable packet. Gate on `> 0` — the same predicate
  // write.js uses (`!max`) — so a non-positive/array value yields 0 at BOTH sites
  // and the guards can't diverge into a bogus-max crash. The effective size is
  // clamped to a broker-side ceiling (never-evicting, client-keyed table — see
  // OUTBOUND_TOPIC_ALIAS_LIMIT); a configurable cap is tracked in #1124.
  const advertisedTopicAliasMax = packet.properties?.topicAliasMaximum
  client._outboundTopicAliasMaximum = packet.protocolVersion === 5 && advertisedTopicAliasMax > 0
    ? Math.min(advertisedTopicAliasMax, OUTBOUND_TOPIC_ALIAS_LIMIT)
    : 0
