...
function init (client, packet, done) {
  ...
  // Only a numeric value is honored: mqtt-packet decodes a duplicated property
  // into an array (a Protocol Error per §3.1.2.11.5), which must not reach the
  // write path as a bogus max. The effective size is clamped to a broker-side
  // ceiling (never-evicting, client-keyed table — see OUTBOUND_TOPIC_ALIAS_LIMIT);
  // a configurable cap is tracked in #1124.
  const advertisedTopicAliasMax = packet.properties?.topicAliasMaximum
  client._outboundTopicAliasMaximum = packet.protocolVersion === 5 && typeof advertisedTopicAliasMax === 'number'
