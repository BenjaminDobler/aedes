...
function init (client, packet, done) {
  ...
  // [#840] MQTT 5.0 outbound Topic Alias: the client's Topic Alias Maximum is the
  // highest alias value it will accept from the broker (§3.1.2.11.5). When > 0,
  // the broker MAY alias outbound PUBLISH topics; track the per-connection map.
  // The effective per-connection maximum is min(client-advertised, broker option)
  // — the broker's `outboundTopicAliasMaximum` bounds the never-evicting,
  // client-keyed table (0 disables outbound aliasing entirely). Require a positive
  ...
  //
  // A duplicated Topic Alias Maximum (§3.1.2.11.5 Protocol Error) is deliberately
  // handled LENIENTLY here — the non-integer array simply disables outbound aliasing
  // for the connection rather than refusing the CONNECT with 0x82. Outbound aliasing
  // is a broker-side MAY optimisation the client can live without, so degrading it is
  // safer than dropping an otherwise-valid connection over a malformed optional
  // property; the receiveMaximum/authenticationData Protocol Errors above stay strict
  // because those affect correctness, not a bandwidth optimisation.
  ...
  if (client._outboundTopicAliasMaximum > 0) {
    client._outboundTopicAliases = new Map()
  }
  ...
  // [#840] A Will carries client-supplied properties, and mqtt-packet round-trips
  ...
  // source too — the inbound-PUBLISH analogue lives at publish.js resolveTopicAlias.
  if (client._will?.properties?.topicAlias !== undefined) {
    delete client._will.properties.topicAlias
  }
