  ...
  if (client.version === 5 && client._authenticationMethod !== undefined && client.broker.authenticateEnhanced) {
    startEnhancedAuth(client, this.packet, {
      ...
      onFailure (err) {
        arg.client = client
        arg.returnCode = 5
        arg.reasonCode = err.reasonCode ?? ReasonCodes.NOT_AUTHORIZED
        if (err.reasonString) {
...
function doConnack (arg, done) {
  ...
  if (version === 5 && arg.returnCode === 0 && arg.reasonCode === undefined && !arg.properties) {
    ...
    // [#833] Final Authentication Data from a successful enhanced-auth exchange.
    // Merged into the generated properties (not passed as arg.properties, which
    // would skip this whole capability block — sharedSubscriptionAvailable, etc.).
