...
import { startEnhancedAuth } from './auth.js'
import { SESSION_NEVER_EXPIRES, RECEIVE_MAXIMUM_DEFAULT, REASON_CODE_ERROR_THRESHOLD, ReasonCodes } from '../constants.js'
...
function init (client, packet, done) {
  ...
  // [§3.1.2.11.9] Shape-check the auth properties at the trust boundary: the
  // Authentication Method (when present) must be a single non-empty UTF-8 string
  // and Authentication Data a single binary value. mqtt-packet decodes a
  // duplicated v5 property into an ARRAY (a Protocol Error), which would otherwise
  // reach `authenticateEnhanced` as method: string[] / data: Buffer[] on the
  // pre-auth path — contradicting the hook's declared contract and steering a
  // length/prefix check off its branch. An empty-string method is also rejected
  // (a hook guarding with `if (method)` would fail open). [#833]
  if (packet.protocolVersion === 5) {
    const authMethod = packet.properties?.authenticationMethod
    const authData = packet.properties?.authenticationData
    if ((authMethod !== undefined && (typeof authMethod !== 'string' || authMethod === '')) ||
        (authData !== undefined && !Buffer.isBuffer(authData))) {
      rejectConnect(client, {
  ...
  // re-authentication. Absent for username/password clients.
  client._authenticationMethod = packet.properties?.authenticationMethod
...
function authenticate (arg, done) {
  ...
  // configured hook runs the AUTH-packet exchange instead of username/password.
  // The exchange spans multiple packets (auth.js); it resolves this connect step
  // via the closures below — success resumes the pipeline (→ doConnack CONNACK
  // 0x00), failure sends a rejection CONNACK like the username/password path.
  if (client.version === 5 && client._authenticationMethod !== undefined && typeof client.broker.authenticateEnhanced === 'function') {
    startEnhancedAuth(client, this.packet, {
      ...
      onFailure (err) {
        arg.client = client
        arg.returnCode = 5
        // A rejection must carry a failure reason code (>= 0x80). Clamp anything
        // a hook mistakenly returns below the threshold (incl. 0x00 SUCCESS) to
        // 0x87, so the wire CONNACK is never SUCCESS-coded on a rejection.
        arg.reasonCode = err.reasonCode >= REASON_CODE_ERROR_THRESHOLD
          ? err.reasonCode
          : ReasonCodes.NOT_AUTHORIZED
        if (err.reasonString) {
          arg.properties = { reasonString: err.reasonString }
        }
        err.errorCode = arg.returnCode
        // Mirror the resolved codes onto the error (as rejectConnect / the redirect
