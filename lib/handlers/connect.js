...
function init (client, packet, done) {
  ...
  // The upper bound is client-advertised only; a broker-side cap for symmetry with
  // the inbound `topicAliasMaximum` option is tracked in #1124.
  client._outboundTopicAliasMaximum = packet.protocolVersion === 5 ? (packet.properties?.topicAliasMaximum ?? 0) : 0
