...
function init (client, packet, done) {
  ...
  if (packet.protocolVersion === 5 && packet.properties?.authenticationMethod !== undefined &&
    ...
    const rawMethod = packet.properties.authenticationMethod
    const requestedMethod = typeof rawMethod === 'string' && rawMethod.length > 64
      ? rawMethod.slice(0, 64) + '…'
      : rawMethod
...
function doConnack (arg, done) {
  ...
  if (client.maximumPacketSize > 0 && connack.properties?.authenticationData !== undefined) {
    const size = mqtt.generate(connack, { protocolVersion: version }).length
    if (size > client.maximumPacketSize) {
