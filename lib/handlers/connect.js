  ...
  if (client.version === 5 && client._authenticationMethod !== undefined && typeof client.broker.authenticateEnhanced === 'function') {
    startEnhancedAuth(client, this.packet, {
      ...
      onFailure (err) {
        ...
        arg.client = client
        arg.returnCode = 5
        // [MQTT-3.2.2-8] A rejection CONNACK MUST carry one of the Connect Reason
        // Codes (Table 3-1). Clamp anything a hook returns that isn't a legal
        // failure code — a success/< 0x80 code, or a real reason code that simply
        // isn't valid on CONNACK (e.g. 0x8E, 0x93) — to 0x87, so the wire CONNACK
        // is never SUCCESS-coded or illegal.
...
function doConnack (arg, done) {
  ...
  // [MQTT-3.1.2-24] Honor the client's Maximum Packet Size on the CONNACK too —
  // it carries the largest hook-controlled field (Authentication Data) — so
  // mqtt-packet drops optional props to fit rather than emitting an over-size one.
  const writeOpts = client.maximumPacketSize > 0
