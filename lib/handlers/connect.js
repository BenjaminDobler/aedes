...
function init (client, packet, done) {
  ...
  if (packet.protocolVersion === 5 && packet.properties?.authenticationMethod !== undefined &&
    ...
    // client asking for a method the broker can't do. Name the requested method.
    const requestedMethod = packet.properties.authenticationMethod
    ...
    startEnhancedAuth(client, this.packet, {
      ...
      onFailure (err) {
        ...
        if (err === null || typeof err !== 'object') {
          err = new Error('enhanced authentication rejected')
        }
        arg.client = client
        arg.returnCode = 5
        ...
        if (client.version === 5 && err.serverReference) {
          ...
          // [MQTT-3.2.2-8] A non-redirect rejection CONNACK MUST carry a Connect
          // Reason Code from Table 3-1. Clamp anything else — a success/< 0x80 code,
          // a code not valid on CONNACK (0x8E, 0x93, …), OR a redirect code (0x9C/
          // 0x9D) with no serverReference to justify it — to 0x87.
          arg.reasonCode = err.reasonCode !== ReasonCodes.USE_ANOTHER_SERVER &&
            err.reasonCode !== ReasonCodes.SERVER_MOVED &&
            CONNACK_FAILURE_REASON_CODES.has(err.reasonCode)
            ? err.reasonCode
...
function doConnack (arg, done) {
  ...
  // [MQTT-3.1.2-24] The client's Maximum Packet Size is NOT enforced on the CONNACK:
  // mqtt-packet's connack() writer calls getProperties (not the size-aware
  // getPropertiesByMaximumPacketSize the auth/disconnect/publish writers use), so a
  // per-call opt would be a silent no-op — and even if it applied, a props-can't-fit
  // `false` return would be read by write() as backpressure and stall the connect in
  // waitForDrain. Outbound CONNACK size is therefore advisory here, consistent with
  // the receiveMaximum / outbound-maximumPacketSize posture documented above. The
  // largest hook-controlled field is the enhanced-auth Authentication Data; a hook
  // targeting a tiny client Maximum Packet Size should keep its final data small.
