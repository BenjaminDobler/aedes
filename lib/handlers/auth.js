...
function handleAuth (client, packet, done) {
  ...
  if (!state) {
    // No exchange in flight; distinguish two cases for a connected client:
    //  - [MQTT-4.12.1-1] a client that negotiated NO Authentication Method at
    //    CONNECT must not send AUTH at all → Protocol Error (0x82).
    //  - one that DID negotiate a method is attempting re-authentication (0x19),
    //    not yet supported (#833 fast-follow) → Implementation specific error
    //    (0x83).
    client.broker.emit('clientError', client, new Error('unexpected AUTH packet'))
    if (client.connected) {
      const reasonCode = client._authenticationMethod === undefined
