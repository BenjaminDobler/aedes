...
function runEnhancedAuthStep (client, data) {
  ...
  try {
    client.broker.authenticateEnhanced(client, state.method, data, function onResult (err, result) {
      ...
      // exchange until the deadline.
      write(client, serverAuthPacket(client, state.method, ReasonCodes.CONTINUE_AUTHENTICATION, result && result.data, result && result.properties), onWriteError)
      client.resume()
...
function handleAuth (client, packet, done) {
  ...
  if (!state) {
    // No exchange in flight. Per [MQTT-4.12.1-1] a genuine re-authentication is
    // specifically reason code 0x19 with the SAME Authentication Method the client
    // negotiated at CONNECT — that single case maps to 0x83 (Implementation
    // specific error, since aedes doesn't yet support re-auth). Everything else —
    // no method negotiated, wrong reason code, or a mismatched method — is a
    // Protocol Error → 0x82 (§4.13.1).
