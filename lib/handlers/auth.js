...
function finishAuth (client, err) {
  ...
  // Success — but a DISCONNECT may have arrived while the hook was pending. The
  // read loop is paused across a step, so those bytes sit unread in the socket
  // buffer; completing now would registerClient (evicting a live same-id session
  // with 0x8E) and CONNACK 0x00 for a client that already left. Drain the buffer
  // to honor a buffered DISCONNECT, and abort the completion if the client
  // disconnected. _enhancedAuth is already nulled, so a stray buffered AUTH can't
  // start a fresh step.
  client.resume()
