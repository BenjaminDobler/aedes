...
function runEnhancedAuthStep (client, data) {
  ...
  function processOutcome (err, result) {
    ...
    if (client.closed || client.broker.closed || client._enhancedAuth !== state) return
    if (err) {
...
function handleAuth (client, packet, done) {
  ...
  if (!state) {
    ...
    const err = new Error('unexpected AUTH packet')
    // Key on connackSent (set before `connected`, with the async emptyQueue drain
    // in between) so an AUTH in that window still gets a DISCONNECT, not a bare
    // close. Per [MQTT-4.12.1-1] a genuine re-authentication is specifically reason
    // code 0x19 with the SAME Authentication Method the client negotiated — that
    // single case maps to 0x83 (Implementation specific error; aedes doesn't yet
    // support re-auth). Everything else is a Protocol Error → 0x82 (§4.13.1).
  ...
  // §4.12: the Authentication Method must not change during the exchange. A
  // changed method is more specifically 0x8C (Bad Authentication Method) than a
