...
// challenge, an uncaught process exit on the CONNACK size probe). `data` must be a
// Buffer if present; from `properties` only reasonString (string) and userProperties
// (object) are forwarded, so validate those and ignore the rest.
...
function finishAuth (client, err) {
  ...
  // 0x00 for a client that already left. A DISCONNECT can be in two places: still in
  // the socket buffer (arrived in a later segment while the read loop was paused) —
  // resume() reads it — OR already parsed into the pre-connected queue (pipelined in
  // the CONNECT's segment, before _enhancedAuth existed) — _dispatchQueuedDisconnect
  // drains it. Honor both, then abort the completion if the client disconnected.
  client.resume()
  client._dispatchQueuedDisconnect()
...
function runEnhancedAuthStep (client, data) {
  ...
  function processOutcome (err, result) {
    ...
    if (!validAuthResult(result)) {
      client.broker.emit('clientError', client, new Error('authenticateEnhanced returned an invalid result (data must be a Buffer; properties.reasonString a string, userProperties an object)'))
    ...
    // completes. _dispatchQueuedDisconnect still lets a pipelined DISCONNECT abort.
