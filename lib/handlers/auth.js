...
function runEnhancedAuthStep (client, data) {
  ...
  // connection (close() cleared nothing, as the timer wasn't armed yet) — or on a
  // broker that is shutting down.
  if (client.closed || client.conn.destroyed || client.broker.closed) return
  const state = client._enhancedAuth
  ...
  function processOutcome (err, result) {
    ...
    // pending. Check both liveness flags, as finishAuth does.
    if (client.closed || client.conn.destroyed || client.broker.closed || client._enhancedAuth !== state) return
    if (err) {
