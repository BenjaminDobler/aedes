...
import { validateTopic, $SYS_PREFIX, runParallel, once, noop, ackProperties } from '../utils.js'
...
class UnSubAck {
  constructor (packet, granted, client) {
    ...
    // [#823] Reason String when at least one topic filter had no matching
    // subscription — the per-topic codes already convey it; this adds
    // human-readable diagnostics (gated by Request Problem Information).
    if (granted.some(code => code === ReasonCodes.NO_SUBSCRIPTION_EXISTED)) {
      const properties = ackProperties(client, 'no subscription existed for one or more topic filters')
...
function completeUnsubscribe (err) {
  ...
  if (packet.messageId !== undefined) {
    write(client, new UnSubAck(packet, this.granted, client),
      done)
  } else {
    done()
  }
