import { runSeries, ackProperties, topicLevelCount } from '../utils.js'
...
function resolveTopicAlias (client, packet) {
  ...
  if (packet.topic.length > 0) {
    ...
    if (packet.topic.indexOf('#') === -1 && packet.topic.indexOf('+') === -1) {
      client._inboundTopicAliases.set(alias, packet.topic)
    }
  } else {
    const topic = client._inboundTopicAliases.get(alias)
    if (topic === undefined) {
