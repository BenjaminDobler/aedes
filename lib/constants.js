...
// [#840] Default broker-side ceiling on how many outbound Topic Aliases the broker
// assigns per connection, regardless of the (client-advertised) Topic Alias
// Maximum. The table never evicts and is keyed by full topic strings, so an
// unbounded client-chosen size is a per-connection memory-amplification vector;
// 64 covers realistic topic sets. Operator-tunable via the `outboundTopicAliasMaximum`
// broker option (0 disables outbound aliasing).
export const OUTBOUND_TOPIC_ALIAS_MAXIMUM_DEFAULT = 64
