...
class Client {
  ...
  close (done) {
    ...
    // Release the per-connection Topic Alias tables promptly (like _parser._queue),
    ...
    // The inbound map is .clear()'d (kept a Map) rather than nulled: resolveTopicAlias
    // has no `!max` short-circuit, and .get() on an empty map safely returns an
    // "unknown alias" error, so this can't throw on a PUBLISH parsed in the same batch.
    this._outboundTopicAliasMaximum = 0
    this._outboundTopicAliases = null
    this._inboundTopicAliases.clear()
