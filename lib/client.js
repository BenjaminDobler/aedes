...
function enqueue (packet) {
  ...
  // Dispatch immediately when the packet is: already connected (post-CONNACK), the
  // first packet of a batch (the initial CONNECT, or a protocol-violating packet
  // that must be rejected), or an AUTH/DISCONNECT continuing an in-flight
  // enhanced-auth exchange. [#833] The exchange resumes the read loop between
  // rounds, so `_parsingBatch === 1` is no longer a safe fast-path there — gate it
  // off while `_enhancedAuth` is set, and route the exchange's AUTH/DISCONNECT via
  // the dedicated clause instead. Any other packet arriving mid-exchange is queued
  // — never processed pre-CONNACK — and drains on 'connected' (only after a
  // successful authentication).
  if (client.connackSent ||
      (client._parsingBatch === 1 && !client._enhancedAuth) ||
      ((packet.cmd === 'auth' || packet.cmd === 'disconnect') && client._enhancedAuth)) {
