...
class Client {
  constructor (broker, conn, req) {
    ...
    this._inboundTopicAliases = new Map()
    // MQTT 5.0 outbound Topic Alias state (broker -> client), negotiated at
    // CONNECT. Declared here (not lazily in connect.js) so write()'s hot path
    // reads a monomorphic shape on every client, and so `_outboundTopicAliasMaximum
    // > 0 ⇒ _outboundTopicAliases is a Map` is a local, enforced invariant.
    this._outboundTopicAliasMaximum = 0
    this._outboundTopicAliases = null
  ...
  close (done) {
    ...
    // Release the outbound Topic Alias table promptly (like _parser._queue),
    // shortening the retention window if anything still references a closed client
    // (conn.client backref, in-flight mqemitter callbacks, a takeover race). [#840]
    //
    // `connected` isn't cleared until finish() runs (after the async
    // handleUnsubscribe), so an in-flight deliver0/deliverQoS (both setImmediate)
    // can still reach write() in this window. Zero the outbound MAXIMUM in the SAME
    // statement so withOutboundTopicAlias short-circuits on `!max` before it touches
    // the now-null Map — never leave the max/map pair inconsistent (the constructor
    // invariant: `_outboundTopicAliasMaximum > 0` ⇒ `_outboundTopicAliases` is a Map).
    //
    // The INBOUND map is deliberately NOT nulled: `resolveTopicAlias` has no `!max`
    // short-circuit to hide behind, and its deref isn't inside write()'s try/catch,
    // so nulling it would risk an uncaught throw from a PUBLISH parsed in the same
    // synchronous batch as the close. Like `subscriptions`/`duplicates` (larger maps
    // that are also not nulled), it is collected with the Client on disconnect.
    this._outboundTopicAliasMaximum = 0
    this._outboundTopicAliases = null
