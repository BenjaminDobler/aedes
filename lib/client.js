...
function enqueue (packet) {
  ...
  // already connected, the first packet, or an AUTH continuation of an in-progress
  // enhanced-auth exchange. [#833] AUTH must be dispatched in the pre-CONNACK
  // window, not queued: the queue only drains on the 'connected' event, which
  // cannot fire until the exchange (waiting on this very AUTH) completes.
  if (client.connackSent || client._parsingBatch === 1 ||
      (packet.cmd === 'auth' && client.connecting)) {
