...
class Client {
  constructor (broker, conn, req) {
    ...
    // [#833] MQTT 5.0 Enhanced Authentication exchange state / deadline. Declared
    // here (not lazily in auth.js) so `enqueue` and `close()` read a monomorphic
    // shape on the 99% of connections that never use enhanced auth.
  ...
  disconnect (opts, done) {
    ...
    // Gate on `connecting || connected` (not just `connected`) — the same window
    // write() serializes in — so a server-initiated DISCONNECT with a reason code
    // (e.g. rejecting a stray AUTH) still reaches the wire in the connackSent-but-
    // not-yet-connected window instead of bare-closing (§4.13.1). [#833]
    if (this.version === 5 && (this.connecting || this.connected) && !this.closed && !this.errored) {
